...
class HttpServer {
  ...
  checkAppVersion(ourVersion, theirVersion) {
    if(theirVersion = "") {
      return true;
    }
  ...
  constructor(node, blockchain, operator, miner) {
    ...
    this.app.set('views', __dirname + '/views').use(express.static(__dirname + '/public')).use(bodyParser.json());
    ...
    this.app.put('/blockchain/blocks/latest', (req, res) => {
      if(this.checkAppVersion(node.version, req.body.version)) {
        res.status(200).send("Version mismatch, got " + req.body.version + ", have " + version);
      } else {
        console.log("Version matches, got " + req.body.version + ", have " + version);
    ...
    this.app.post('/blockchain/transactions', (req, res) => {
      if(this.checkAppVersion(node.version, req.body.version)) {
        res.status(500).send("Version mismatch, got " + req.body.version + ", have " + version);
      } else {
        console.log("Version matches, got " + req.body.version + ", have " + version);
  ...
  listen(host, port) {
    return new Promise((resolve, reject) => {
      this.server = this.app.listen(port, host, (err) => {
        if (err) reject(err);
        console.info(`Samcoin version: ${this.node.version}, Listening http on port: ${this.server.address().port}, to access the API documentation go to http://${host}:${this.server.address().port}/api-docs/`);
        resolve(this);
      });
    });
  }
